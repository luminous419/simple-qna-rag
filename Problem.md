...
이 문서는 현재 해결되지 않은 문제만 관리합니다. 해결된 문제의 상세 이력은 Git 커밋과 Pull Request에서 확인합니다. 마일스톤 목표와 우선순위는 [Roadmap.md](Roadmap.md), 현재 품질 수치는 [M2 최초 기준선](evaluation/baselines/m2_initial.md)을 참조하십시오.
...
`document_register.py`는 기존 `vectorstore/`를 삭제하고 전체 인덱스를 다시 생성합니다. 증분 인덱싱, 버전 관리, 원자적 교체와 자동 복구가 없습니다.
