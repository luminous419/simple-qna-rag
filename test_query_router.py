...
class TestRouteQueryKeywordDetection:
    def test_web_search_keyword_routes_to_web_search(self):
        with patch(
            "query_router.search_and_format", return_value=_mock_web_result()
        ) as mock_search, patch("query_router.get_rag_engine") as mock_get_engine:
    ...
    def test_no_keyword_routes_to_document_qa(self):
        ...
        ) as mock_get_engine, patch("query_router.search_and_format") as mock_search:
    ...
    def test_search_query_strips_keyword_boilerplate(self):
        with patch(
            "query_router.search_and_format", return_value=_mock_web_result()
        ) as mock_search, patch("query_router.get_rag_engine"):
...
class TestRouteQueryFeatureFlag:
    def test_web_search_disabled_ignores_keywords(self):
        mock_engine = MagicMock()
        mock_engine.query.return_value = _mock_rag_result()
        with patch("query_router.USE_WEB_SEARCH", False), patch(
            "query_router.get_rag_engine", return_value=mock_engine
        ) as mock_get_engine, patch("query_router.search_and_format") as mock_search:
            result = query_router.route_query("웹검색으로 Python을 찾아줘")
