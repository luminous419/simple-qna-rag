...
from simple_qna_rag.web_search import format_search_results, search_and_format, search_web
...
class TestSearchWeb:
    def test_returns_formatted_results(self):
        with patch("simple_qna_rag.web_search.DDGS", return_value=_mock_ddgs(SAMPLE_RESULTS)):
    ...
    def test_missing_fields_get_defaults(self):
        with patch("simple_qna_rag.web_search.DDGS", return_value=_mock_ddgs([{}])):
    ...
    def test_exception_returns_empty_list(self):
        with patch("simple_qna_rag.web_search.DDGS", side_effect=RuntimeError("network error")):
            results = search_web("테스트 쿼리")
