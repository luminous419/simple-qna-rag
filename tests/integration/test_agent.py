...
class TestRouteQueryWebSearchDisabled:
    def test_bypasses_agent_entirely_when_disabled(self):
        ...
        with patch("simple_qna_rag.agent.USE_WEB_SEARCH", False), patch(
            "simple_qna_rag.agent.get_rag_engine", return_value=mock_engine
        ) as mock_get_engine, patch("simple_qna_rag.agent._decide_tool") as mock_decide:
...
class TestRouteQueryToolSelection:
    def test_web_search_tool_selected_and_succeeds(self):
        with patch("simple_qna_rag.agent.USE_WEB_SEARCH", True), patch(
            "simple_qna_rag.agent._decide_tool", return_value=("web_search", "정제된 검색어")
        ), patch.object(
    ...
    def test_document_qa_tool_selected(self):
        with patch("simple_qna_rag.agent.USE_WEB_SEARCH", True), patch(
            "simple_qna_rag.agent._decide_tool", return_value=("document_qa", "RAG에서 MMR이 뭐야?")
        ), patch.object(
            agent.rag_tool, "func", return_value=_rag_result()
...
class TestRouteQueryFallback:
    def test_decide_tool_exception_falls_back_to_keyword_router(self):
        with patch("simple_qna_rag.agent.USE_WEB_SEARCH", True), patch(
            "simple_qna_rag.agent._decide_tool", side_effect=RuntimeError("ollama down")
        ), patch(
            "simple_qna_rag.agent.keyword_fallback_route", return_value=_web_result()
        ) as mock_fallback:
            result = agent.route_query("아무 질문")
    ...
    def test_no_tool_selected_falls_back_to_keyword_router(self):
        with patch("simple_qna_rag.agent.USE_WEB_SEARCH", True), patch(
            "simple_qna_rag.agent._decide_tool", return_value=(None, None)
        ), patch(
            "simple_qna_rag.agent.keyword_fallback_route", return_value=_web_result()
        ) as mock_fallback:
            result = agent.route_query("아무 질문")
...
class TestRouteQueryFallbackDocumentQaRetry:
    ...
    def test_decide_tool_exception_with_keyword_web_search_failure_retries_document_qa(self):
        ...
        mock_engine.query.return_value = _rag_result()
        with patch("simple_qna_rag.agent.USE_WEB_SEARCH", True), patch(
            "simple_qna_rag.query_router.USE_WEB_SEARCH", True
        ), patch(
            "simple_qna_rag.agent._decide_tool", side_effect=RuntimeError("ollama down")
        ), patch(
