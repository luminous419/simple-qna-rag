...
상태: **구현 완료 — Code Review Iteration 9 PASS 10.0/10(`CRITICAL 0/MAJOR 0/
MINOR 0/TRIVIAL 0`), 이 세션(통합/인수 worker)이 실제 로컬 이미지 build/scan/
smoke를 최초로 완주해 로컬 결정론적 검증 전 항목 PASS. hosted CI/merge 증거는
여전히 NOT_RUN(미커밋)**\
기준 revision: `648e3ab` (작업 트리 변경 미커밋 — no commit/push/PR)  
근거: [Requirement.md](Requirement.md), [Plan.md](Plan.md), [Design.md](Design.md),
[Implementation_Report.md](Implementation_Report.md) §1~15,
[Code_Review_Iteration_1.md](Code_Review_Iteration_1.md)~
[Code_Review_Iteration_9.md](Code_Review_Iteration_9.md)와 그 remediation 문서,
...
**Code Review Iteration 6~9 closure(2026-08-14 이전, 별도 세션들 — 상세는
[Implementation_Report.md](Implementation_Report.md) §14)**: `scan_image_layers.py`의
certifi 코멘트 Label 바인딩을 둘러싼 4회의 독립 리뷰 cycle이 이어졌다 —
Iteration 6(FAIL 9.4/10, CR-I6-MAJ-01 인증서 미바인딩) → remediation(6개
필드 exact binding 추가, Label만 미해결) → Iteration 7(FAIL 9.5/10,
CR-I7-MAJ-01 Label만 대소문자/공백 정규화) →
[remediation](Code_Review_Iteration_7_Remediation.md)(Label byte-exact
비교 + 2-entry 예외 테이블) → Iteration 8(FAIL 9.5/10, CR-I8-MAJ-01 예외
테이블이 리뷰 환경의 진짜 pip-vendored `certifi==2023.07.22` bundle을
반영하지 못함) →
[remediation](Code_Review_Iteration_8_Remediation.md)(3-bundle 호환 경계
명시 + 5개 entry 추가, 총 8-entry) → **Iteration 9(PASS 10.0/10,
`CRITICAL 0/MAJOR 0/MINOR 0/TRIVIAL 0`)** — 독립 리뷰어가 세 bundle 전체를
자신의 grammar/decoder로 다시 walk해 8-entry 테이블이 정확히 그 합집합과
일치함을 확인했다. 이 cycle 전체에서 변경된 파일은
`scripts/scan_image_layers.py`/`tests/unit/test_scan_image_layers.py`/
`tests/unit/test_rag_engine_singleton.py` 3개뿐이며, Requirement/Plan/Design/
workflow/baseline checker는 `84f6b407` 대비 diff 0으로 유지됐다 —
`M4.1_BLOCKED=true`/protected M3 live `NOT_RUN`/`operational_status=BLOCKED`/
`overall_release_ready=false` 불변식은 전혀 영향받지 않았다. 이 절 및 아래
표의 M4.3-REQ-005/NFR-003 행은 Iteration 9 PASS와 이 세션이 재실행한
검증(§ 아래, 실제 로컬 이미지 build/scan/smoke 최초 완주 포함)을 반영해
갱신했다.
...
| M4.3-NFR-003 보안 | contained-open/pickle prevalidation 로컬 실측 확인; OCI layer scan 로직 PASS(실제 이미지로 반복 실측, 이 세션이 최초로 로컬 build/scan/smoke 전체 완주); remediation 이후: pre-verification 읽기가 선언 크기+1로 bounded(무제한 메모리 적재 결함 제거), crash-recovery journal이 strict-schema fail-closed(corrupted journal이 PASS로 승격되던 결함 제거); Code Review Iteration 5 remediation 이후: certifi 코멘트 grammar가 정확히 7필드/순서/중복/필드별 문법을 강제(임의 텍스트 밀반입 결함 제거), `RAGEngine` 실패 싱글톤/artifact reason이 재시도에서 살아남지 않고 `EngineArtifactError`가 공개 reason 허용목록을 강제; Code Review Iteration 6~9 이후: 7개 필드 전부(Issuer/Subject/Serial/MD5/SHA1/SHA256/Label) 실제 인증서에 byte-exact 바인딩(Label의 대소문자/공백 정규화 결함 제거), 3-bundle 호환 경계가 8-entry exact `(SHA-256, Label)` 테이블로 명시·독립 검증됨(**PASS 10.0/10**) | **PASS(로컬)** |
...
| Container hosted | PR + master push | build, security/mock smoke, layer scan | `docker build --target test`/`--target production`(linux/amd64), `scan_image_layers.py`(`forbidden_count: 0`), `container_smoke.py`(`status: PASS`, 전 필드 true) — 통합/인수 세션이 실제 로컬 이미지로 처음 끝까지 완주(이전 세션들의 반복적인 Docker Desktop VM 디스크 소진이 이번에는 재현되지 않음); hosted receipt는 여전히 `NOT_RUN`(첫 실제 hosted x86_64 실행 확인 필요) |
